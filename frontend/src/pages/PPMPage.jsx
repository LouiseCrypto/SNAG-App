...
export default function PPMPage() {
  ...
  const [editNotesJob, setEditNotesJob] = useState(null)  // job whose notes are being edited
  const [editNotesText, setEditNotesText] = useState('')

  const fetchJobs = () =>
    axios.get(`${API}/ppm?engineer_id=${engineer?.id}`).then(r => setJobs(r.data)).catch(() => {})
  ...
  const handleHold = async () => {
    ...
    try {
      await axios.post(`${API}/ppm/${holdModal.id}/hold`, { engineer_id: engineer.id, note: holdNote })
  ...
  const handleResume = async (job) => {
    try {
      await axios.post(`${API}/ppm/${job.id}/resume`, { engineer_id: engineer.id })
  ...
  const handleEditNotes = async () => {
    try {
      await axios.patch(`${API}/ppm/${editNotesJob.id}/notes`, { engineer_id: engineer.id, notes: editNotesText })
  ...
  const addJob = async (e) => {
    ...
    try {
      await axios.post(`${API}/ppm`, { ...newJob, engineer_id: engineer.id })
  ...
  // Backend already filtered by engineer_id — show only this engineer's jobs
    ...
    <div className="space-y-5 animate-fadeIn">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">Showing only your assigned jobs</p>
        </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-start gap-4">
              <div className="flex-1">
                <div className="flex items-start justify-between gap-2 mb-1">
                  ...
                  <span className={STATUS_COLOR[job.status] || 'badge-gray'}>{job.status}</span>
                </div>
